...
st.set_page_config(page_title="Martingale Difference Divergence (MDD) with Macro", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Martingale Difference Divergence with Macro</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Kernel‑based measure of predictability from macro variables</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 MDD")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'mdd_macro_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>MDD score: {etf['mdd_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized MDD"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized MDD", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Martingale Difference Divergence (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Martingale Difference Divergence (MDD)** measures how much the conditional expectation of returns given macro deviates from zero.
        - A martingale difference sequence has E[return_t | past macro] = 0. High MDD indicates predictability.
        - We use a Gaussian kernel on standardised macro variables to compute a quadratic form: MDD = Σ_{i≠j} K(macro_i, macro_j) * ret_i * ret_j.
        - Higher MDD → macro strongly predicts returns → potential alpha source.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the MDD scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Martingale Difference Divergence | Kernel‑based predictability from macro")
